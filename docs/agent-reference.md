# GnomeGarden Agent Reference

This file preserves the detailed examples and generated usage rules that used to live in `AGENTS.md`. The root `AGENTS.md` is the concise operating contract and wins on any conflict. Read this reference only when you need detailed syntax examples or expanded framework guidance; do not treat stale workflow wording here as higher priority than `AGENTS.md`.
...
Read the top project-specific sections first and treat the generated
`usage-rules` block at the bottom as detailed syntax/framework reference. When
instructions overlap, prefer the more specific GnomeGarden/Ash rule over generic
Phoenix defaults, especially for persisted business behavior and Ash-backed
forms.

Before broad edits:

1. Check `git status --short` and preserve existing user changes.
2. Identify the owning domain/resource from `docs/llm/index.md`,
   `docs/llm/generated/resources.json`, and `config :gnome_garden, :ash_domains`.
3. State the current coherent milestone, the Ash-first design point, and focused
   validation before changing many files. Prefer sustained progress through an
   approved milestone over stopping after tiny incremental changes, while still
   keeping the milestone bounded enough to review as one coherent diff.
4. Keep writes single-threaded in the active worktree. Use subagents/Codex for
   research, planning, and review unless writers are isolated in git worktrees.

## Research And Validation Budget

Do enough research to avoid guessing, but do not turn every small edit into a
full repo audit.

- For dependency behavior, generators, Ash DSL, migrations, Oban, Cinder,
  AshStorage, Phoenix/LiveView, or unexpected errors, search package docs and/or
  run `mix help` first.
- For simple documentation, copy, styling, or localized tests that do not touch
  framework behavior, inspect the local pattern and keep moving.
- Batch related edits before broad validation. Prefer focused tests or compile
  checks for the touched slice, then `mix precommit` only for final PR-ready
  checkpoints or when requested.
- If context gets large, update the durable planning/runbook doc you are using
  instead of relying on chat history.

## Critical: Documentation Lookup

Search docs before implementing dependency or framework behavior. Use these mix tasks:
...
For Ash-backed create/update forms, use `AshPhoenix.Form` rather than Ecto
changesets or ad hoc param handling. Plain `to_form/1` remains fine for local
UI-only filters/search forms that do not create or update Ash resources.
...
3. **Protect existing work** - inspect `git status --short` before edits and do
   not overwrite unrelated user changes.
4. **Generate migrations**: `mix ash.codegen`.
5. **Apply migrations**: `mix ash.migrate`.
6. **Refresh the LLM resource map after Ash domain/resource changes**:
   `mix llm.generate_resource_map`.
7. **During development**: batch related edits before running format/tests.
...
8. **Before opening a PR only**: run `mix precommit` as the final broad check.

For large or long-running tasks, keep a short durable work note in the relevant
`docs/` plan/runbook with goal, constraints, current milestone, validation
commands, open decisions, files changed, and residual risks. Do not stop after
tiny incremental changes when an approved milestone can be completed safely.
After large generated diffs, run or request a fresh-context review before
applying follow-up fixes.
