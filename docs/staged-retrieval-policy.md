...
server-rendered sources use HTTP then browser; authenticated PlanetBids uses Jido browser; BidNet
uses its Playwright adapter; SAM.gov uses its API. Browserless is never selected implicitly.
