...
attributes:
  - id: uuid PK
  - email: :ci_string, allow_nil?: false, unique
  - organization_id: uuid, FK → organizations, allow_nil?: false

AshAuthentication magic_link strategy:
...
Portal LiveViews use a dedicated `on_mount: :require_client_user` hook (separate from staff `:live_user_optional`). `current_client_user` is always set and passed as `actor` to every Ash query.
...
scope "/portal" do
  ...
  # AshAuthentication magic link callback handled automatically
end
```
...
ash_authentication_live_session :client_portal,
  on_mount: :require_client_user do
...
config :gnome_garden, stripe_webhook_secret: System.get_env("STRIPE_WEBHOOK_SECRET")
```
...
**Invoice issue hook:** When `Finance.issue_invoice/1` is called, after state transition:
```elixir
case StripeClient.create_payment_link(invoice) do
  {:ok, url} → Finance.update_invoice(invoice, %{stripe_payment_url: url})
  {:error, _} → Logger.warning(...) # non-fatal, ACH still works
end
```
...
- Handles `checkout.session.completed` → looks up invoice by `metadata.invoice_id` → calls `Finance.mark_paid(invoice)`
...
AshAuthentication also generates a `client_user_tokens` table (handled automatically).
...
Same pattern on `Agreement`. URL tampering (e.g., changing invoice ID) returns a not-found error, never returns another org's data.
...
lib/garden_web/live/client_portal_auth.ex          ← require_client_user hook
```
