defmodule GnomeGarden.Agents.Tools.Commercial.SaveDiscoveryFindingTest do
  ...
  test "stores discovered discovery-record data in operations and acquisition intake" do
    {:ok, result} =
      SaveDiscoveryFinding.run(
        %{
    ...
    assert discovery_record.organization_id == organization.id
    assert discovery_record.contact_person_id == person.id
    assert discovery_record.name == "North Coast Packaging"
    assert discovery_record.website_domain == "northcoastpackaging.com"
    assert discovery_record.fit_score >= 70
    assert discovery_record.intent_score >= 65
    assert metadata_value(discovery_record.metadata, :source) == "save_discovery_finding"

    assert observation.discovery_record_id == discovery_record.id
    ...
    assert metadata_value(observation.metadata, :source) == "save_discovery_finding"
  end
  ...
  test "attaches discovered records to a discovery program when provided" do
    ...
    assert metadata_value(discovery_record.metadata, :discovery_program_id) ==
  ...
  test "logs discovery-record outputs onto the originating agent run when run context is present" do
    ...
    {:ok, deployment} =
      Agents.create_agent_deployment(%{
        name: "SaveDiscoveryFinding Test Deployment #{System.unique_integer([:positive])}",
        visibility: :private,
        enabled: true,
        config: %{},
        source_scope: %{},
    ...
    output = Enum.find(outputs, &(&1.output_type == :finding))

    assert output
    assert output.output_id == result.finding_id
    assert output.event == :created
    assert output.label == "West Basin Foods"
    assert metadata_value(output.metadata, :finding_id) == result.finding_id
    assert metadata_value(output.metadata, :discovery_record_id) == result.discovery_record_id
    assert metadata_value(output.metadata, :evidence_id) == result.evidence_id
  end
