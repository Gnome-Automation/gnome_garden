defmodule GnomeGarden.Agents.Tools.ProjectInfo do
  @moduledoc """
  ...
  use Jido.Action,
    name: "project_info",
    description:
      "Get information about the current project. Detects project type, structure, and configuration.",
    schema: [
  ...
  defp git_info(path) do
    case System.cmd("git", ["rev-parse", "--show-toplevel"], cd: path, stderr_to_stdout: true) do
      {_, 0} ->
        {branch, _} =
          System.cmd("git", ["branch", "--show-current"], cd: path, stderr_to_stdout: true)

        {remote, _} =
          System.cmd("git", ["remote", "get-url", "origin"], cd: path, stderr_to_stdout: true)
