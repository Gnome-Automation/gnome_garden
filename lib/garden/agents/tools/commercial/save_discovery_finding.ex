defmodule GnomeGarden.Agents.Tools.Commercial.SaveDiscoveryFinding do
  @moduledoc """
  Save a discovered company into the durable acquisition discovery-record model.

  Agent intake creates or updates the durable organization record first,
  optionally records a contact person, then creates or updates a durable
  discovery record plus supporting evidence for human review before anything
  ...
  def run(params, context) do
    ...
    with {:ok, organization_resolution} <- resolve_organization(params, actor),
         ...
         {:ok, finding} <-
           Acquisition.sync_discovery_record_finding(discovery_record, actor: actor) do
      log_discovery_record_output(
        context,
        finding,
        discovery_record,
        discovery_evidence,
        existing_discovery_record,
        existing_discovery_evidence
      )
      ...
      {:ok,
       %{
         ...
         evidence_id: discovery_evidence.id,
         finding_id: finding.id,
         company: params.company_name,
         message: "Discovery record saved for commercial review."
       }}
    else
      {:error, reason} ->
        {:error, "Failed to save discovery record: #{inspect(reason)}"}
    end
  end

  defp existing_discovery_record(params) do
    params[:website]
  ...
  defp upsert_discovery_record(
    ...
    attrs = %{
      ...
      fit_score: discovery_score.fit_score,
      intent_score: discovery_score.intent_score,
      status: :new,
      notes: build_discovery_record_notes(params),
      ...
      metadata: %{
        ...
        discovery_program_id: blank_to_nil(params[:discovery_program_id]),
        source: "save_discovery_finding",
        market_focus: %{
          company_profile_key: profile_context.company_profile_key,
          company_profile_mode: profile_context.company_profile_mode,
          icp_matches: discovery_score.icp_matches,
          risk_flags: discovery_score.risk_flags,
          fit_rationale: discovery_score.fit_rationale,
          intent_signals: discovery_score.intent_signals
        },
        contact_snapshot: DiscoveryIdentityResolver.discovery_contact_snapshot(params),
        identity_review:
  ...
  defp upsert_discovery_evidence(
         params,
         discovery_record,
         person,
         discovery_score,
    ...
    attrs = %{
      ...
      confidence_score: discovery_score.intent_score,
      summary: summary,
  ...
  defp build_discovery_record_notes(params) do
  ...
  defp observation_external_ref(params) do
    ...
    "save_discovery_finding:#{company_ref}:#{signal_ref}:#{source_ref}"
  end
  ...
  defp discovery_record_upsert_identity(%{website: website}) when is_binary(website),
    do: :unique_website_domain

  defp discovery_record_upsert_identity(%{location: location}) when is_binary(location),
    do: :unique_name_key_location

  defp discovery_record_upsert_identity(_attrs), do: nil

  defp log_discovery_record_output(
         context,
         finding,
         discovery_record,
         discovery_evidence,
    ...
    event =
      cond do
        existing_discovery_evidence -> :existing
        existing_discovery_record -> :updated
        true -> :created
      end

    RunOutputLogger.log(context, %{
      output_type: :finding,
      output_id: finding.id,
      event: event,
      label: discovery_record.name,
      summary: "#{event_label(event)} discovery finding #{discovery_record.name}",
      metadata: %{
        finding_id: finding.id,
        discovery_record_id: discovery_record.id,
