defmodule GnomeGarden.Agents.StreamingHandler do
  @moduledoc """
  ...
  def handle_event([:jido, :ai, :tool, :execute, :stop] = event, measurements, metadata, _config) do
    ...
    Logger.info(
      "[StreamingHandler] Tool complete: #{tool_name} for agent #{agent_id}, duration=#{inspect(duration)}"
    )
    ...
    if agent_id && tool_name do
      ...
      duration_ms =
        cond do
          is_nil(duration) -> nil
          # nanoseconds to ms
          duration > 1_000_000 -> div(duration, 1_000_000)
          # already in ms
          true -> duration
        end
  ...
  defp truncate_result(result) when is_map(result),
    do: inspect(result, limit: 5, printable_limit: 200)
