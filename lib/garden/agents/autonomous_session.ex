defmodule GnomeGarden.Agents.AutonomousSession do
  @moduledoc """
  ...
  def subscribe(pid) do
    {:ok, state} = status(pid)
    Phoenix.PubSub.subscribe(GnomeGarden.PubSub, "autonomous:#{state.id}")
  end
    ...
    case Jido.AgentServer.start_link(jido: GnomeGarden.Jido, agent: Base) do
      {:ok, pid} ->
        ...
        if agent_id do
          ...
          Phoenix.PubSub.subscribe(GnomeGarden.PubSub, topic)
        else
  ...
  def handle_info({:stream, {:llm_complete, %{thinking: thinking, text: _text}}}, state) do
    Logger.debug(
      "[AutonomousSession] Received LLM complete, thinking=#{inspect(thinking != nil)}"
    )
  ...
  defp extract_thinking(%{thinking_content: thinking})
  ...
  defp complete(state, reason, result \\ nil) do
    new_state = %{state | status: :completed, completed_at: DateTime.utc_now(), result: result}
  ...
  defp broadcast(id, message) do
    Phoenix.PubSub.broadcast(GnomeGarden.PubSub, "autonomous:#{id}", message)
  end
