defmodule GnomeGarden.Commercial.Review do
  @moduledoc """
  ...
  def accept_review_item(params, opts \\ []) do
    ...
    with {:ok, signal} <- resolve_signal(params, actor),
         ...
         {:ok, _source} <- advance_source_for_pursuit(params, signal, actor),
  ...
  defp resolve_bid_signal(bid_id, actor) do
    with {:ok, bid} <- Procurement.get_bid(bid_id, actor: actor, load: [:signal]) do
      case bid.signal do
        ...
        nil -> resolve_bid_finding_signal(bid.id, actor)
      end
    end
  end

  defp resolve_bid_finding_signal(bid_id, actor) do
    case Acquisition.get_finding_by_source_bid(bid_id, actor: actor) do
      {:ok, finding} ->
        resolve_finding_signal(finding.id, actor)
    ...
    with {:ok, finding} <- Acquisition.get_finding(finding_id, actor: actor) do
      cond do
        ...
        finding.status == :accepted ->
          promote_finding_signal(finding, actor)

        true ->
          {:error, "finding must be accepted in acquisition before commercial pursuit"}
      end
    end
  end

  defp promote_finding_signal(finding, actor) do
    with {:ok, %{finding: promoted_finding}} <-
           Acquisition.promote_finding_to_signal(finding.id, actor: actor),
         signal_id when is_binary(signal_id) <- promoted_finding.signal_id do
      Commercial.get_signal(signal_id, actor: actor)
    else
      nil -> {:error, "promoted finding did not create a commercial signal"}
      error -> error
    end
  end
  ...
  defp advance_source_for_pursuit(params, signal, actor) do
    case value(params, :bid_id) || signal.metadata["procurement_bid_id"] do
      nil -> {:ok, nil}
      bid_id -> advance_bid_for_pursuit(bid_id, actor)
    end
  end

  defp advance_bid_for_pursuit(bid_id, actor) do
    with {:ok, bid} <- Procurement.get_bid(bid_id, actor: actor) do
      case bid.status do
        status when status in [:new, :reviewing] ->
          Procurement.pursue_bid(bid, actor: actor)
        ...
        status when status in [:won, :lost, :expired, :rejected, :parked] ->
          {:error, "bid cannot be pursued from #{status}"}
      end
    end
  end
