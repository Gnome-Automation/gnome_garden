defmodule GnomeGarden.Acquisition.ResearchRequest do
  @moduledoc """
  Research request for acquisition validation.

  Tracks research tasks for organizations, people, bids, findings, and discovery records.
  ...
  actions do
    ...
    read :for_organization do
      ...
      filter expr(
               researchable_type == "organization" and researchable_id == ^arg(:organization_id)
             )
    ...
    read :for_person do
      ...
      filter expr(researchable_type == "person" and researchable_id == ^arg(:person_id))
      prepare build(sort: [inserted_at: :desc])
    end
  end
  ...
  attributes do
    ...
    attribute :researchable_type, :string do
      allow_nil? false
      public? true
      description "Type of entity: organization, person, bid, finding, discovery_record"
    end
  ...
  relationships do
    ...
    many_to_many :bids, GnomeGarden.Procurement.Bid do
      through GnomeGarden.Acquisition.ResearchLink
    ...
    many_to_many :events, GnomeGarden.Commercial.Event do
      through GnomeGarden.Acquisition.ResearchLink
      source_attribute_on_join_resource :research_request_id
