defmodule GnomeGarden.Banking.Actions.BuildBankSyncHistoryWorkspace do
  @moduledoc """
  Builds the provider sync-history workspace: recent sync runs and integration
  events with health counts.
  """
  ...
  def run(_input, _opts, context) do
    ...
    with {:ok, sync_runs} <- Banking.list_recent_bank_sync_runs(actor: actor),
         {:ok, integration_events} <- Banking.list_bank_integration_event_history(actor: actor) do
      {:ok,
       %{
         sync_runs: sync_runs,
