defmodule GnomeGarden.Mercury.SyncWorker do
    ...
    case Providers.Mercury.list_accounts() do
      {:ok, body} ->
        ...
        results =
          Enum.map(raw_accounts, fn raw ->
            ...
            case Mercury.get_mercury_account_by_mercury_id(attrs.mercury_id,
              ...
              {:ok, existing} ->
                Mercury.update_mercury_account(existing, Map.delete(attrs, :mercury_id),
                  authorize?: false
                )
        ...
        if errors == [] do
          ...
          Logger.warning(
            "MercurySyncWorker: #{length(errors)} account upsert error(s): #{inspect(errors)}"
          )
  ...
  defp sync_transactions(accounts) do
    ...
    new_count =
      Enum.reduce(accounts, 0, fn account, acc ->
  ...
  defp sync_account_transactions(account, start_date, bank_rules) do
    Logger.info(
      "MercurySyncWorker: fetching transactions for #{account.name} (#{account.mercury_id})"
    )
    ...
    case Providers.Mercury.list_transactions(account.mercury_id, start_date: start_date) do
            ...
            {:error, _} ->
              ...
              case Mercury.create_mercury_transaction(attrs, authorize?: false) do
                {:ok, txn} ->
                  ...
                  if matched_rule do
                    Mercury.update_mercury_transaction(
                      txn,
                      %{
                    ...
                    Logger.info(
                      "BankRules: applied rule '#{matched_rule.name}' to transaction #{txn.mercury_id}"
                    )
                  else
      ...
      {:error, reason} ->
        Logger.warning(
          "MercurySyncWorker: could not fetch transactions for account #{account.mercury_id}: #{inspect(reason)}"
        )
