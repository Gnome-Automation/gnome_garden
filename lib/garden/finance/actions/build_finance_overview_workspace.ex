defmodule GnomeGarden.Finance.Actions.BuildFinanceOverviewWorkspace do
  @moduledoc """
  Builds the stable top-level Finance overview context.

  The overview is intentionally a synthesis of existing Finance workspaces so
  the LiveView can render one shape without coordinating multiple resource
  reads itself.
  """
  ...
  def run(_input, _opts, context) do
    ...
    with {:ok, banking} <- Finance.get_banking_workspace(actor: actor),
  ...
  defp sync_action(%{failed_sync_count: count}) when count > 0 do
    %{
      title: "Review failed bank sync",
      description: "#{count} recent sync run failed.",
  ...
  defp review_action(%{needs_review_count: count}) when count > 0 do
    %{
      title: "Review bank activity",
      description: "#{count} bank transactions need a decision.",
      path: "/finance/banking/review",
  ...
  defp overdue_action(%{overdue_invoice_count: count, overdue_balance_total: total})
       when count > 0 do
    %{
      title: "Follow up on overdue invoices",
      description: "#{count} invoices overdue, #{format_amount(total)} outstanding.",
  ...
  defp bill_action(%{source_group_count: count, ready_total: total}) when count > 0 do
    %{
      title: "Prepare billable work",
      description: "#{count} customer groups ready, #{format_amount(total)} unbilled.",
  ...
  defp rules_action(%{bank_rules: []}) do
    %{
      title: "Add bank automation rules",
      description: "Create the first rules for recurring deposits, fees, and transfers.",
  ...
  defp format_amount(%Decimal{} = amount),
