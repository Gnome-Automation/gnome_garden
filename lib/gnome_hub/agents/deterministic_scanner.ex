defmodule GnomeHub.Agents.DeterministicScanner do
  @moduledoc """
  ...
  alias GnomeHub.Agents.Tools.Browser.{Navigate, Extract}
  alias GnomeHub.Agents.Tools.{ScoreBid, SaveBid}
  ...
  def scan(lead_source_id) when is_binary(lead_source_id) do
    case Ash.get(LeadSource, lead_source_id) do
      {:ok, %{discovery_status: :discovered, scrape_config: config} = source} when config != %{} ->
        do_scan(source)
  ...
  def scan_all_ready(opts \\ []) do
    ...
    sources = Ash.read!(LeadSource, action: :ready_for_scan, action_input: %{since_hours: since_hours})
  ...
  defp save_qualifying_bids(scored_bids, source) do
    qualifying = Enum.filter(scored_bids, fn bid ->
      score = bid["score"]
      score && score.score_total >= 50
    end)

    saved =
      Enum.map(qualifying, fn bid ->
        score = bid["score"]
