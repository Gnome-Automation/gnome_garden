defmodule GnomeHub.Agents.Workers.Sales.BidScanner do
  ...
  use Jido.AI.Agent,
    ...
    tools: [
      # Scanning tools
      GnomeHub.Agents.Tools.ScanPlanetBids,
      GnomeHub.Agents.Tools.QuerySamGov,
  ...
  """
  def scan_type(pid, source_type, opts \\ []) when source_type in [:planetbids, :sam_gov, :opengov] do
    query = """
  ...
  defp load_lead_sources do
    case Ash.read(GnomeHub.Agents.LeadSource, filter: [enabled: true]) do
      {:ok, sources} ->
