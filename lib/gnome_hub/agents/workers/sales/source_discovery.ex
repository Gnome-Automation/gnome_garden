defmodule GnomeHub.Agents.Workers.Sales.SourceDiscovery do
  ...
  use Jido.AI.Agent,
    ...
    tools: [
      ...
      GnomeHub.Agents.Tools.AnalyzePage,
      GnomeHub.Agents.Tools.SaveLeadSource,
  ...
  @oc_cities [
    "Mission Viejo", "Laguna Niguel", "Aliso Viejo", "Dana Point",
    "San Juan Capistrano", "Laguna Hills", "Rancho Santa Margarita",
    "Lake Forest", "Foothill Ranch", "Ladera Ranch", "Yorba Linda",
    "Brea", "La Habra", "Placentia", "Orange", "Villa Park", "Seal Beach",
    "Los Alamitos", "Cypress", "La Palma", "Buena Park", "Stanton"
  ]

  # LA County cities
  @la_cities [
    "Torrance", "Carson", "Compton", "Downey", "Norwalk", "Whittier",
    "Montebello", "Pico Rivera", "El Monte", "West Covina", "Pomona",
    "Azusa", "Glendora", "Covina", "Diamond Bar", "Walnut", "La Verne",
    "Claremont", "San Dimas", "Arcadia", "Monrovia", "Duarte", "Irwindale"
  ]
  ...
  """
  def discover_industry(pid, industry, opts \\ []) when industry in [:water, :school, :port, :utility] do
    search_terms = case industry do
  ...
  defp load_existing_sources do
    case Ash.read(GnomeHub.Agents.LeadSource) do
      {:ok, sources} ->
        sources
        |> Enum.map(& &1.url)
        |> MapSet.new()
