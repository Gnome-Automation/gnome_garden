defmodule GnomeHub.Agents.Tools.SaveBid do
  @moduledoc """
  ...
  defp find_existing(%{url: url}) do
    case Ash.read(GnomeHub.Agents.Bid, filter: [url: url]) do
  ...
  defp create_bid(params) do
    ...
    case Ash.create(GnomeHub.Agents.Bid, attrs) do
      {:ok, bid} ->
        Logger.info("[SaveBid] Created bid: #{bid.title} (score: #{bid.score_total}, tier: #{bid.score_tier})")
        {:ok, %{
          id: bid.id,
          title: bid.title,
