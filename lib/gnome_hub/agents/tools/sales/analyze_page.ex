defmodule GnomeHub.Agents.Tools.AnalyzePage do
  @moduledoc """
  ...
  use Jido.Action,
    name: "analyze_page",
    description: "Analyze a web page to determine if it's a procurement portal worth adding as a lead source",
    schema: [
  ...
  defp fetch_page(url) do
    case Req.get(url,
      headers: [{"user-agent", "GnomeHub SourceDiscovery/1.0"}],
      max_redirects: 3,
  ...
  defp detect_portal_type(url, content) do
    Enum.reduce_while(@portal_signatures, {:unknown, 0}, fn {type, signatures}, acc ->
      hits = Enum.count(signatures, fn sig ->
