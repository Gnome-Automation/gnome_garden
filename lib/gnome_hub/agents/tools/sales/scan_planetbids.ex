defmodule GnomeHub.Agents.Tools.ScanPlanetBids do
  @moduledoc """
  ...
  use Jido.Action,
    ...
    schema: [
      portal_id: [type: :string, required: true, doc: "PlanetBids portal ID (e.g., '47688' for Irvine)"],
  ...
  defp fetch_and_parse([url | rest], portal_id) do
    case Req.get(url, headers: [{"user-agent", "GnomeHub BidScanner/1.0"}]) do
  ...
  defp parse_bid_row(row_html, source_url, portal_id) do
    ...
    department = extract_text(row_html, ~r/<td[^>]*class="[^"]*department[^"]*"[^>]*>(.*?)<\/td>/s)
    ...
    case Jason.decode(html) do
      {:ok, %{"data" => bids}} when is_list(bids) ->
        Enum.map(bids, fn bid ->
          %{
            external_id: bid["bidNumber"] || bid["id"] || "pb-#{portal_id}-#{:erlang.phash2(bid)}",
