defmodule GnomeHub.Agents.Tools.BrowseWeb do
  @moduledoc """
  Fetch and read web pages using a headless browser.

  Supports content extraction, screenshot capture, and link extraction.
  Uses jido_browser under the hood.
  """

  use Jido.Action,
    name: "browse_web",
    description: "Fetch and read web pages using a headless browser. Supports content extraction, screenshot capture, and link extraction.",
    schema: [
      url: [
        type: :string,
        required: true,
  ...
  def run(params, _context) do
    ...
    action = Map.get(params, :action) || Map.get(params, "action", "get_content")
  ...
  defp truncate(content), do: binary_part(content, 0, @max_content_bytes) <> "\n\n[... truncated at 10KB ...]"
