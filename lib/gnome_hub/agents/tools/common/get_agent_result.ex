defmodule GnomeHub.Agents.Tools.GetAgentResult do
  @moduledoc """
  ...
  use Jido.Action,
    ...
    description: "Wait for a spawned child agent to finish its task and return the result. Use this after spawn_agent to collect the output.",
    schema: [
  ...
  alias GnomeHub.Agents.AgentTracker
  ...
  defp do_wait(agent_id, start_time, timeout, poll_interval) do
    case AgentTracker.get_agent(agent_id) do
      nil ->
        {:error, "Agent '#{agent_id}' not found. It may have already completed and been cleaned up."}
