defmodule GnomeHub.Agents.Tools.SaveDiscovery do
  @moduledoc """
  ...
  use Jido.Action,
    ...
    schema: [
      ...
      listing_url: [type: :string, required: true, doc: "URL of the bid listings page (after navigation)"],
      listing_selector: [type: :string, required: true, doc: "CSS selector for bid rows (e.g., 'table.bids tr', '.bid-item')"],
      ...
      description_selector: [type: :string, doc: "CSS selector for description if visible in listing"],
      agency_selector: [type: :string, doc: "CSS selector for agency/department"],
      pagination_type: [type: :string, doc: "Pagination type: 'numbered', 'load_more', 'infinite', 'none'"],
      pagination_selector: [type: :string, doc: "CSS selector for pagination (next button or page links)"],
        ...
        case Ash.update(source, %{scrape_config: scrape_config}, action: :save_discovery) do
          {:ok, updated} ->
            {:ok, %{
              ...
              message: "Scraping config saved! Future scans will use deterministic scraping (no LLM)."
            }}
