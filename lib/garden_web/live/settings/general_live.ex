defmodule GnomeGardenWeb.Settings.GeneralLive do
  ...
  def mount(_params, _session, socket) do
    settings =
      case Finance.get_billing_settings(actor: socket.assigns.current_user, authorize?: false) do
        {:ok, [s | _]} -> s
        _ -> %{session_timeout_minutes: 30, portal_session_timeout_minutes: 30}
      end
    ...
    {:ok,
     ...
     |> assign(:portal_session_timeout_minutes, settings.portal_session_timeout_minutes)
  ...
  def handle_event("save", params, socket) do
    with {staff, ""} <- Integer.parse(Map.get(params, "session_timeout_minutes", "30")),
         true <- staff >= 0 and staff <= 480,
         {portal, ""} <- Integer.parse(Map.get(params, "portal_session_timeout_minutes", "30")),
         true <- portal >= 0 and portal <= 480 do
      case Finance.upsert_billing_settings(
             %{session_timeout_minutes: staff, portal_session_timeout_minutes: portal},
        ...
        {:ok, _} ->
          {:noreply,
           socket
           |> assign(:session_timeout_minutes, staff)
           |> assign(:portal_session_timeout_minutes, portal)
           |> assign(:save_ok, true)
           |> assign(:save_error, nil)}
      ...
      _ -> {:noreply, assign(socket, :save_error, "Both values must be between 0 and 480.")}
    end
  end
    ...
    <.page max_width="max-w-2xl" class="pb-8">
      ...
      <div class="rounded-xl border border-gray-900/10 dark:border-white/10 p-6">
        ...
        <p class="mt-1 text-sm/6 text-gray-600 dark:text-gray-400">
          How long users can be idle before being logged out automatically. Set to 0 to disable.
        </p>
        ...
        <div class="mt-6 border-t border-gray-900/10 dark:border-white/10 pt-6">
          <form phx-submit="save" class="space-y-6">
            ...
            <div class="flex items-center gap-4">
              <label for="portal_session_timeout_minutes" class="block text-sm/6 font-medium text-gray-900 dark:text-white w-64">
                Client portal timeout
              </label>
              <div class="flex items-center gap-2">
                <input
                  type="number"
                  id="portal_session_timeout_minutes"
                  name="portal_session_timeout_minutes"
                  value={@portal_session_timeout_minutes}
                  min="0"
                  max="480"
                  class={@input_class}
                />
