defmodule GnomeGardenWeb.Acquisition.FindingLive.Index do
  ...
  def handle_event("transition", %{"id" => id, "action" => "accept"}, socket) do
    case Acquisition.accept_finding_review(id, actor: socket.assigns.current_user) do
      {:ok, _finding} ->
        {:noreply, socket |> refresh_queue() |> put_flash(:info, "Marked finding as accepted")}
            ...
            >
              <div class="flex flex-wrap items-start justify-between gap-3">
                ...
                <div class="space-y-2 text-right">
                  ...
                  <.status_badge
                    :if={@selected_source.last_run_state}
                    status={@selected_source.last_run_state_variant}
                  >
                    {status_label(@selected_source.last_run_state)}
                  </.status_badge>
                </div>
              </div>
            ...
            >
              <div class="flex flex-wrap items-start justify-between gap-3">
                ...
                <div class="space-y-2 text-right">
                  ...
                  <.status_badge
                    :if={@selected_program.last_run_state}
                    status={@selected_program.last_run_state_variant}
                  >
                    {status_label(@selected_program.last_run_state)}
                  </.status_badge>
                </div>
              </div>
              <div class="mt-3 flex flex-wrap gap-2">
              ...
              <tr :for={{dom_id, finding} <- @streams.findings} id={dom_id}>
                ...
                <td class="px-3 py-2.5 align-top text-zinc-600 dark:text-zinc-300">
                  <div class="space-y-1">
                    ...
                    <p
                      :if={finding_due_hint(finding)}
                      class="text-xs text-zinc-500 dark:text-zinc-400"
                    >
                      {finding_due_hint(finding)}
                    </p>
                  </div>
                </td>
                <td class="px-3 py-2.5 align-top">
                  <div class="space-y-1">
                    ...
                    <p
                      :if={finding_score_note(finding)}
                      class="text-xs text-zinc-500 dark:text-zinc-400"
                    >
                      {finding_score_note(finding)}
                    </p>
                    <span
                      :if={finding_score_tier(finding)}
                      class={score_tier_badge(finding_score_tier(finding))}
                    >
                      {status_label(finding_score_tier(finding))}
                    </span>
                  </div>
                </td>
                <td class="px-3 py-2.5 align-top text-zinc-600 dark:text-zinc-300">
                  <div class="space-y-1">
                    <p class="font-medium text-zinc-900 dark:text-white">
                      {finding_location_label(finding)}
                    </p>
                    <p
                      :if={finding_location_note(finding)}
                      class="text-xs text-zinc-500 dark:text-zinc-400"
                    >
                      {finding_location_note(finding)}
                    </p>
                  </div>
                </td>
                <td class="px-3 py-2.5 align-top text-zinc-600 dark:text-zinc-300">
                  <div class="space-y-1">
                    <p class="font-medium text-zinc-900 dark:text-white">
                      {finding_work_label(finding)}
                    </p>
                    <p
                      :if={finding_work_detail(finding)}
                      class="text-xs text-zinc-500 dark:text-zinc-400"
                    >
                      {finding_work_detail(finding)}
                    </p>
                    <p
                      :if={finding_work_note(finding)}
                      class="max-w-[14rem] text-xs text-zinc-500 dark:text-zinc-400"
                    >
                      {finding_work_note(finding)}
                    </p>
                  </div>
                </td>
                <td class="px-3 py-2.5 align-top text-zinc-600 dark:text-zinc-300">
                ...
                <td class="px-3 py-2.5 align-top">
                  <div class="flex flex-wrap gap-1.5">
                    ...
                    <.button
                      :if={
                        finding.status in [:new, :reviewing, :accepted] and is_nil(finding.signal_id)
                      }
  ...
  defp finding_due_label(finding) do
    case finding_due_at(finding) do
      nil -> "No due date"
      due_at -> format_date(due_at)
    end
  end

  defp finding_due_hint(%{source_bid: %{due_at: %DateTime{} = due_at}}) do
    if DateTime.compare(due_at, DateTime.utc_now()) == :lt,
      do: "Past due",
      else: "Procurement deadline"
  end

  defp finding_due_hint(_finding), do: nil

  defp finding_due_at(%{source_bid: %{due_at: %DateTime{} = due_at}}), do: due_at

  defp finding_due_at(%{metadata: metadata}) when is_map(metadata) do
    metadata
    |> metadata_value("due_at")
    |> parse_datetime_value()
  end

  defp finding_due_at(_finding), do: nil
  ...
  defp finding_score_note(%{intent_score: score}) when is_integer(score), do: "Intent #{score}"

  defp finding_score_note(%{confidence: confidence}) when not is_nil(confidence),
    do: "#{confidence_label(confidence)} confidence"

  defp finding_score_note(_finding), do: nil

  defp finding_score_tier(%{source_bid: %{score_tier: tier}}) when not is_nil(tier), do: tier
  defp finding_score_tier(_finding), do: nil

  defp score_tier_badge(:hot), do: "badge badge-success badge-sm"
  defp score_tier_badge(:warm), do: "badge badge-warning badge-sm"
  defp score_tier_badge(:prospect), do: "badge badge-info badge-sm"
  defp score_tier_badge(_tier), do: "badge badge-ghost badge-sm"

  defp finding_location_label(%{source_bid: %{location: location}})
       when is_binary(location) and location != "",
       do: location

  defp finding_location_label(%{source_discovery_record: %{location: location}})
       when is_binary(location) and location != "",
       do: location

  defp finding_location_label(%{metadata: metadata}) when is_map(metadata) do
    metadata_value(metadata, "location") || "No location"
  end

  defp finding_location_label(_finding), do: "No location"

  defp finding_location_note(%{source_bid: %{region: region}}) when not is_nil(region),
    do: humanize_token(to_string(region))

  defp finding_location_note(%{source_discovery_record: %{region: region}})
       when is_binary(region) and region != "",
       do: region

  defp finding_location_note(_finding), do: nil

  defp finding_work_label(%{source_bid: %{score_icp_matches: [match | _]}})
       when is_binary(match),
       do: humanize_token(match)

  defp finding_work_label(%{source_discovery_record: %{industry: industry}})
       when is_binary(industry) and industry != "",
       do: industry

  defp finding_work_label(finding), do: type_label(finding.finding_type)

  defp finding_work_detail(%{source_bid: %{bid_type: bid_type}}) when not is_nil(bid_type),
    do: humanize_token(to_string(bid_type))

  defp finding_work_detail(%{source_discovery_record: %{size_bucket: size_bucket}})
       when not is_nil(size_bucket),
       do: humanize_token(to_string(size_bucket))

  defp finding_work_detail(_finding), do: nil

  defp finding_work_note(%{source_bid: %{agency: agency}})
       when is_binary(agency) and agency != "",
       do: agency

  defp finding_work_note(%{watchouts: [watchout | _]}) when is_binary(watchout),
    do: "Watchout: #{watchout}"

  defp finding_work_note(_finding), do: nil
  ...
  defp metadata_value(%{} = metadata, "due_at"),
    do: Map.get(metadata, "due_at") || Map.get(metadata, :due_at)

  defp metadata_value(%{} = metadata, "location"),
    do: Map.get(metadata, "location") || Map.get(metadata, :location)

  defp metadata_value(_metadata, _key), do: nil

  defp parse_datetime_value(%DateTime{} = value), do: value
