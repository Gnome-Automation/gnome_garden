defmodule GnomeGardenWeb.AgentLive do
  @moduledoc """
  LiveView for interacting with GnomeGarden AI agents.
  Supports both chat mode and autonomous goal-driven mode.
  """
  use GnomeGardenWeb, :live_view

  alias GnomeGarden.Agents.Workers.Base
  alias GnomeGarden.Agents.AutonomousSession
  ...
  def handle_event("send", %{"message" => message}, socket)
      when message != "" and socket.assigns.mode == :chat do
    socket =
      if socket.assigns.agent_pid == nil do
        case Jido.AgentServer.start_link(jido: GnomeGarden.Jido, agent: Base) do
          {:ok, pid} ->
            Process.monitor(pid)
  ...
  def handle_info({:iteration, number, result}, socket) do
    {text, is_error} =
      case result do
        ...
        {:error, :timeout} ->
          {"The agent timed out on this iteration. This can happen with complex operations.",
           true}

        {:error, e} ->
          {"Error: #{inspect(e)}", true}
  ...
  def handle_info({:streaming_tool_complete, tool_info}, socket) do
    tool_calls =
      socket.assigns.tool_calls ++
        [Map.put(tool_info, :iteration, socket.assigns.current_iteration + 1)]
  ...
  def handle_info({:DOWN, _ref, :process, pid, reason}, socket) do
    cond do
      ...
      pid == socket.assigns.session_pid ->
        ...
        socket =
          if reason != :normal and socket.assigns.session_status != :completed do
            socket
            |> assign(:session_status, :error)
  ...
  def render(assigns) do
    ...
    <div class="flex flex-col h-screen w-full px-6 py-4">
      <header class="mb-4">
        <h1 class="text-2xl font-bold text-gray-800">GnomeGarden Agent</h1>
    ...
    <% else %>
      <div class="flex-1 flex flex-col">
        ...
        <div class="flex-1 flex gap-4 overflow-hidden">
          ...
          <div class={[
            "flex-1 overflow-y-auto border rounded-lg bg-gray-50 p-4 space-y-3",
            if(@show_thinking, do: "w-1/2", else: "w-full")
          ]}>
            ...
            <%= for iteration <- @iterations do %>
              <div class={[
                ...
                if(iteration[:is_error],
                  do: "bg-red-50 border border-red-200",
                ...
                <div class={[
                  "text-sm whitespace-pre-wrap max-h-96 overflow-y-auto",
                  if(iteration[:is_error], do: "text-red-700", else: "text-gray-700")
                ]}>
                  {iteration.content}
                </div>
                ...
                <div class="flex-1 overflow-y-auto p-2 space-y-1">
                  <%= if @active_tool do %>
                    <div class="text-xs bg-emerald-100 rounded p-2 border border-emerald-300 animate-pulse">
                      <div class="flex items-center gap-2">
                        <div class="animate-spin h-3 w-3 border-2 border-emerald-500 border-t-transparent rounded-full">
                        </div>
