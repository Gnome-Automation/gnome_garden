defmodule GnomeGardenWeb.Finance.BankAccountLive do
  ...
  def handle_event("sync_account", _params, socket) do
    case socket.assigns do
      ...
      %{account: account, bank_connection: connection} when not is_nil(connection) ->
        AshOban.run_trigger(connection, :sync)
  ...
  defp load_workspace(socket, id) do
    case Banking.get_bank_account_workspace(id, actor: socket.assigns.current_user) do
      {:ok, workspace} ->
        socket
