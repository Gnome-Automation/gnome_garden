defmodule GnomeGardenWeb.Finance.BankingLive do
  ...
  def handle_event("sync", _params, socket) do
    case Banking.list_active_bank_connections(actor: socket.assigns.current_user) do
      {:ok, [_ | _] = connections} ->
        Enum.each(connections, &AshOban.run_trigger(&1, :sync))
      ...
      {:ok, []} ->
        {:noreply, put_flash(socket, :error, "No active bank connection to sync.")}
  ...
  defp load_workspace(socket) do
    workspace = Banking.get_banking_workspace!(actor: socket.assigns.current_user)
