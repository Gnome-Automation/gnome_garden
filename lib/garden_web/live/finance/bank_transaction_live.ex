defmodule GnomeGardenWeb.Finance.BankTransactionLive do
  ...
  def handle_event("accept_match", %{"id" => id}, socket) do
    with {:ok, match} <-
           Finance.get_bank_transaction_match(id, actor: socket.assigns.current_user),
         {:ok, _match} <-
           Finance.accept_bank_transaction_match(
             match,
  ...
  def handle_event("reject_match", %{"id" => id}, socket) do
    with {:ok, match} <-
           Finance.get_bank_transaction_match(id, actor: socket.assigns.current_user),
         {:ok, _match} <-
           Finance.reject_bank_transaction_match(
             match,
  ...
  def handle_event("create_rule", _params, socket) do
    ...
    case Finance.create_bank_rule_from_transaction(transaction.id,
           actor: socket.assigns.current_user
         ) do
      {:ok, %{rule: rule}} ->
        {:noreply,
  ...
  defp load_workspace(socket, id) do
    case Finance.get_bank_transaction_workspace(id, actor: socket.assigns.current_user) do
      {:ok, workspace} ->
