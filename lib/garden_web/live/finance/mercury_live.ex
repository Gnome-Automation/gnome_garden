defmodule GnomeGardenWeb.Finance.MercuryLive do
  ...
  def handle_event("open_reconcile_modal", %{"txn_id" => txn_id}, socket) do
    txn = Enum.find(socket.assigns.transactions, &(&1.id == txn_id))
    {:noreply, socket |> assign(:reconciling_txn, txn) |> assign(:reconciliation_note, "") |> assign(:reconciliation_error, nil)}
  end

  @impl true
  def handle_event("close_reconcile_modal", _params, socket) do
    {:noreply, socket |> assign(:reconciling_txn, nil) |> assign(:reconciliation_note, "") |> assign(:reconciliation_error, nil)}
  end

  @impl true
  def handle_event("reconciliation_note_changed", %{"value" => note}, socket) do
    {:noreply, assign(socket, :reconciliation_note, note)}
  end

  @impl true
  def handle_event("confirm_reconcile", _params, socket) do
    note = String.trim(socket.assigns.reconciliation_note)
    ...
    if note == "" do
      ...
      Mercury.update_mercury_transaction(txn, %{match_confidence: :exact, reconciliation_note: note})
  ...
  def handle_event("unmatch_transaction", %{"txn_id" => txn_id}, socket) do
    ...
    Enum.each(invoice_ids, fn invoice_id ->
      case Finance.get_invoice(invoice_id, actor: actor) do
      ...
      <form phx-change="filter_changed" class="mb-4 flex flex-wrap items-end gap-4">
        ...
        <div>
          ...
          <input
            ...
            class="mt-1 block rounded-md bg-white px-3 py-1.5 text-base text-gray-900 outline-1 -outline-offset-1 outline-gray-300 hover:bg-gray-50 focus:outline-2 focus:-outline-offset-2 focus:outline-emerald-600 sm:text-sm/6 dark:bg-white/5 dark:text-white dark:outline-white/10 dark:hover:bg-white/10 dark:focus:outline-emerald-500 transition-colors cursor-pointer"
          />
        </div>
        <div>
        ...
        <div>
          <label for="filter_status" class="block text-sm/6 font-medium text-gray-900 dark:text-white">
            Status
          </label>
          <div class="mt-1 grid grid-cols-1">
            <select
              id="filter_status"
              name="status_filter"
              class="col-start-1 row-start-1 appearance-none rounded-md bg-white py-1.5 pr-8 pl-3 text-base text-gray-900 outline-1 -outline-offset-1 outline-gray-300 hover:bg-gray-50 focus:outline-2 focus:-outline-offset-2 focus:outline-emerald-600 sm:text-sm/6 dark:bg-white/5 dark:text-white dark:outline-white/10 dark:hover:bg-white/10 dark:focus:outline-emerald-500 transition-colors cursor-pointer"
            >
        ...
        <div>
          ...
          <button type="button" phx-click="reset_filters" title="Clear all filters and return to the default 30-day view" class="mt-1 cursor-pointer rounded-md bg-white py-1.5 px-3 text-sm text-gray-900 outline-1 -outline-offset-1 outline-gray-300 hover:bg-gray-50 active:bg-gray-100 dark:bg-white/5 dark:text-white dark:outline-white/10 dark:hover:bg-white/10 dark:active:bg-white/15 transition-colors">
            Reset filters
          </button>
        </div>
      </form>
            ...
            <tbody class="divide-y divide-zinc-200 dark:divide-white/10">
              <tr :for={txn <- @transactions}>
                ...
                <td class="px-5 py-4">
                  ...
                  <p :if={txn.reconciliation_note} class="mt-1 text-xs text-base-content/40 italic">
                ...
                <td class="px-5 py-4 text-right">
                  <div class="flex items-center justify-end gap-2">
                    ...
                    <button
                      :if={txn.match_confidence in [:unmatched, nil] and Decimal.compare(applied, Decimal.new("0")) == :gt}
                      phx-click="open_reconcile_modal"
                      phx-value-txn_id={txn.id}
                      title="Mark remaining balance as reconciled with a required explanation"
      ...
      >
        ...
        >
          ...
          <div class="mt-4">
            ...
            <textarea
              phx-keyup="reconciliation_note_changed"
              placeholder="e.g. Overpayment — credited to next invoice, Bank rounding difference, Client memo discrepancy..."
              rows="3"
