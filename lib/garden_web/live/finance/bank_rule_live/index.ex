defmodule GnomeGardenWeb.Finance.BankRuleLive.Index do
  ...
  def handle_event("toggle_rule", %{"id" => id}, socket) do
    ...
    result =
      if rule.enabled do
        Finance.disable_bank_rule(rule, actor: socket.assigns.current_user)
      else
        Finance.enable_bank_rule(rule, actor: socket.assigns.current_user)
      end
  ...
  def handle_event("delete_rule", %{"id" => id}, socket) do
    ...
    case Finance.delete_bank_rule(rule, actor: socket.assigns.current_user) do
      {:ok, _rule} ->
        {:noreply,
         socket
  ...
  defp load_rules(socket) do
    case Finance.list_bank_rules(actor: socket.assigns.current_user) do
      {:ok, rules} ->
        socket
        |> assign(:rules, rules)
  ...
  defp load_rule!(id, actor) do
    case Finance.get_bank_rule(id, actor: actor) do
      {:ok, rule} -> rule
